...
void Pedeonumero() {
  ...
  while (!Serial.available());
  a2 = Serial.read() - 48; // Lê o primeiro dígito do segundo número
  while (!Serial.available());
  b2 = Serial.read() - 48; // Lê o segundo dígito do segundo número
  while (!Serial.available());
  c2 = Serial.read() - 48; // Lê o terceiro dígito do segundo número

  num1 = a * 100 + b * 10 + c; // Calcula o primeiro número a partir dos dígitos lidos
  num2 = a2 * 100 + b2 * 10 + c2; // Calcula o segundo número a partir dos dígitos lidos
}
